...
object AddYearYamlProtocol extends SharedYamlProtocol {
  implicit val preprocPerPatEnvDataFIPSYamlFormat = yamlFormat4(AddYearConfig)
}

/**
  *  split preagg into individual files for patients 
  */
object AddYear extends StepConfigConfig {

  type ConfigType = PerPatEnvDataFIPSConfig

  val yamlFormat = AddYearYamlProtocol.preprocPerPatEnvDataFIPSYamlFormat

  val configType = classOf[PerPatEnvDataFIPSConfig].getName()

  val log = Logger.getLogger(getClass.getName)

  log.setLevel(Level.INFO)

  def step(spark: SparkSession, config: PerPatEnvDataFIPSConfig) = {
    time {
      ...
      val df_with_year = df.withColumn(config.year_index, year(config.date_index)
